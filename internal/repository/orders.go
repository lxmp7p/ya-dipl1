...
func (rep *Repository) GetOrderByNumber(ctx context.Context, orderNumber string) (*OrderData, error) {
	query := `
...
func (rep *Repository) UpdateOrderStatus(ctx context.Context, orderNumber string, status string, accrual *float64) error {
	query := `
        UPDATE orders 
	...
	_, err := rep.Db.Exec(ctx, query, orderNumber, status, accrual)
	return err
}

func (rep *Repository) BeginTx(ctx context.Context) (pgx.Tx, error) {
